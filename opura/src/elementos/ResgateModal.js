...
export default function ResgateModal({ beneficio, onClose, userId }) {
  ...
  useEffect(() => {
    console.log('userId no ResgateModal:', userId);  // Verifique se o userId está correto
    const getUserPoints = async () => {
      try {
        const pointsResponse = await fetchUserPoints(userId);
        setUserPoints(pointsResponse);
  ...
  const handleResgatar = async () => {
    ...
    try {
      await redeemBenefit(beneficio.id, userId, beneficio.pontos);  // Realiza o resgate do benefício
      const newPoints = userPoints - beneficio.pontos;
      await updateUserPoints(userId, newPoints);  // Atualiza os pontos do usuário corretamente com userId e newPoints
      setUserPoints(newPoints);
      router.push('/resgatar');
    } catch (error) {
